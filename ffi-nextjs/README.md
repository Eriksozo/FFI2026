...
Este projeto usa **`output: 'export'`** — `next build` gera um site 100% estático
na pasta `out/` (sem servidor Node, sem adaptadores).

```bash
npm run build      # gera ./out
npx serve out      # (opcional) testar localmente
```

## Deploy

O site é estático, então roda em qualquer host de arquivos estáticos.

**Cloudflare (Workers/Pages):** o repositório já inclui um `wrangler.jsonc`
configurado para servir a pasta `out/` como assets estáticos — sem OpenNext,
sem Worker dinâmico, sem service bindings.
- Build command: `npm run build`
- Deploy command: `npx wrangler deploy`

**Vercel:** importe o repositório → Deploy (detecta Next.js automaticamente).

**GitHub Pages / Netlify / qualquer host:** publique o conteúdo da pasta `out/`.

> `node_modules/`, `.next/` e `out/` já estão no `.gitignore`.
